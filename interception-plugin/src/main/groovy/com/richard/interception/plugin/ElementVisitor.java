...
import com.richard.interception.plugin.basic.ModuleRunnerWrite;
import com.richard.interception.plugin.bean.ElementModel;
import com.richard.interception.plugin.bean.InterceptionModel;
import com.richard.interception.plugin.utils.VisitorUtils;
import org.objectweb.asm.*;

import static org.objectweb.asm.Opcodes.*;
import static com.richard.interception.plugin.utils.VisitorUtils.*;
...
//        String name = moduleName + "$" + loadName + (isSurround ? "$SurroundLoader" : "$ElementLoader");
    ...
    private static void buildInit(MethodVisitor mv, String elementOwner, String runnerOwner, int processId) {
        mv.visitFieldInsn(GETSTATIC, elementOwner, "HOST_CLASS", "Ljava/lang/String;");
        visitIntPushStack(mv, processId);
        mv.visitMethodInsn(INVOKESTATIC, runnerOwner, "getBuilderById", "(Ljava/lang/String;I)Lcom/richard/interception/internal/IElementMultipartBuilder;", false);
        mv.visitTypeInsn(CHECKCAST, "com/richard/interception/internal/IElementBuilder");
        mv.visitVarInsn(ASTORE, 3);
    ...
    private static void buildNormal(MethodVisitor mv, List<ElementModel> models) {
        for (int i = 0; i < models.size(); i++) {
            ...
            mv.visitInsn(DUP);
            visitIntPushStack(mv, model.getType());
            mv.visitLdcInsn(model.getName());
            mv.visitLdcInsn(new Integer(model.getProcessId()));
            visitLdcInsn(mv, model.getType() == 0 ? "V" : model.getReturnType());
            if (model.getType() == 2) {
                visitIntPushStack(mv, model.getDesc().length);
                mv.visitTypeInsn(ANEWARRAY, "java/lang/Class");
                for (int j = 0; j < model.getDesc().length; j++) {
                    Type type = model.getDesc()[j];
                    mv.visitInsn(DUP);
                    visitIntPushStack(mv, j);
                    visitLdcInsn(mv, type.getDescriptor());
    ...
    private static void buildSurround(MethodVisitor mv, List<ElementModel> models) {
        ...
        {
            //初始化
            visitIntPushStack(mv, models.size());
            ...
            for (int i = 0; i < models.size(); i++) {
                ...
                mv.visitInsn(DUP);
                visitIntPushStack(mv, i);
                visitLdcInsn(mv, model.getReturnType());
                mv.visitInsn(AASTORE);
            }
        }
        {
    ...
    private static void addModelAnnotation(MethodVisitor mv, ElementModel model) {
        visitIntPushStack(mv, model.getInterceptor().size());
        mv.visitTypeInsn(ANEWARRAY, "java/lang/Class");
        ...
        if (type.equals("boolean")) {
            ...
            if (type.equals("int")) {
                ...
                if (fromDefault) {
                    ...
                    visitIntPushStack(mv, v.length);
                    mv.visitIntInsn(NEWARRAY, T_INT);
                    for (int i = 0; i < v.length; i++) {
                        mv.visitInsn(DUP);
                        visitIntPushStack(mv, i);
                        visitIntPushStack(mv, Integer.parseInt(v[i]));
                        mv.visitInsn(IASTORE);
                    }
                } else {
                    int[] v = (int[]) value;
                ...
                if (fromDefault) {
                    ...
                    if (value instanceof List) {
                        ...
                        visitIntPushStack(mv, list.size());
                        String t = type.replace(".", "/");
                        mv.visitTypeInsn(ANEWARRAY, t);
    ...
    private static void buildMethod(String runnerOwner, String elementOwner, String owner, MethodVisitor mv, List<ElementModel> models) {
        ...
        for (int i = 0; i < models.size(); i++) {
            ...
            if (method) {
                ...
                mv.visitInsn(ICONST_0);
                mv.visitInsn(AALOAD);
                visitCheckCast(mv, model.getReturnType());
